...
Read, in this order:
1. `CLAUDE.md` (root) for standing rules, the conductor prompt structure (there is no tier system — see below), and terminology.
...
**Score against the LIVE prompt, not the code constant.** The founder live-edits Jove's prompt from the admin "Tuning" page (`/admin/prompt-architecture`). The text in force is resolved as `voiceOverrides?.conductorPrompt ?? CONDUCTOR_PROMPT` (`src/lib/persona/system-prompt.ts`), reading an enabled row (key `conductor_prompt`) from `persona_voice_overrides` (`voice-overrides.ts`). A transcript was produced under whatever override was live THEN — when reviewing one, query the enabled override read-only and score against it. `CONDUCTOR_PROMPT` in `conductor-prompt.ts` is the permanent floor and version history, not necessarily the text that produced the session. Scoring against the wrong prompt manufactures false findings.

The engine itself, by file:
- **System prompt** — `src/lib/persona/system-prompt.ts`. `buildSystemPromptBlocks` returns three blocks: the conductor prompt (the whole voice, from `conductor-prompt.ts`), compressed older Manual entries, and current-session context. There is NO tier system — the three-tier machinery (Tier 1/2/3) was deleted 2026-07-06 (ADR-052). The `tier1` field name survives in the code as a label for the conductor-prompt block only; do not read it as a constitutional tier.
...
- **Alliance signals — the engine logs none.** The per-turn monitor (`monitor.ts`, the `monitor_reads` table) was removed under ADR-045; do not review against it and do not prescribe rebuilding it — the re-entry condition is consumer-first (name what consumes the signal before building any sensor, and prefer detecting inside the main Jove call over a parallel watcher). The `observation_miss_count` and `pattern_engaged` fields that once lived in `extraction.ts` were deleted on 2026-07-03 (commits `4b2941aa`, `9af646a5`) when the extraction schema was stripped to what the save-time composer and safety detectors actually read. `pattern_engaged` now survives only in stale comments (`persona-pipeline.ts`) and in the two-layer-engine doc — not in code. There is currently no live alliance, rupture, or observation-landed signal on `extraction_state`. Your transcript read is therefore the ONLY place alliance and rupture get assessed — treat that as the state of the world, not a gap to close by prescribing a new sensor (any new sensor must clear the Complexity Gate first).
- **Pipeline, extraction, capture, checkpoints, Manual context** — `persona-pipeline.ts`, `extraction.ts`, `call-persona.ts` (the capture path: Jove emits the `---reflection-ready---` marker mid-response; `call-persona.ts` strips it and tags the message row `metadata.reflection_landed` — this is the pull-only landed signal. There is no `detect-checkpoint.ts`; it was deleted), `confirm-checkpoint.ts`, `manual-context.ts`. Entry composition now has a live A/B path (`composer-mode.ts` / `compose-as-conductor.ts`): an entry may be written by the classic outside composer OR by the conductor itself from full live context — relevant when reviewing why an entry reads the way it does.
...
You are deliberately rubric-blind: when used as an independent cross-check of an `/evaluate` scoring run, do not read `docs/reference/conductor-scoring.md` — your value is a second, differently-grounded read. A shared rubric collapses the independence.
...
The transcript comes to you pasted in USER:/JOVE: form, or as a conversation ID you can pull from the database read-only. When a conversation ID is available, cross-check your read against what the engine DID log on `conversations.extraction_state`: `depth` (surface→behavior→feeling→mechanism→origin), `checkpoint_gate.distinct_contexts` (distinct lived scenes narrated), `sage_brief`, `current_thread`, `clinical_flag`, and `language_bank`. These are a single latest-state snapshot, not a per-turn log — compare against the session's end state, not turn-by-turn. None of them is an alliance or rupture signal (the engine logs none). So where your read of withdrawal or rupture diverges from, say, a deep `depth` reading, that divergence is itself a finding: the extraction model scored the material deep while the user was actually complying or withdrawing. Separately, the pull-only landed signal — `metadata.reflection_landed` on the message row where Jove emitted `---reflection-ready---` — marks where Jove judged a reflection had landed; a landed marker over a passage you read as compliance is a high-value finding.
...
- **Confrontation markers.** Push-back, correction, frustration with Jove. Often easier to repair than withdrawal once recognized. The engine logs no rupture signal at all — neither confrontation nor withdrawal is captured anywhere in `extraction_state` — so your read is the only place either one is registered.
...
- **The per-turn alliance monitor was removed (ADR-045, 2026-06-04), and the extraction-side alliance signals followed on 2026-07-03.** `monitor.ts` and the `monitor_reads` table no longer exist. `observation_miss_count` and `pattern_engaged` — which the prior version of this note called the surviving signals — were deleted from `src/lib/persona/extraction.ts` on 2026-07-03 (commits `4b2941aa`, `9af646a5`) when the extraction schema was stripped to what the composer and safety detectors read. There is now NO alliance/rupture/observation-landed signal anywhere in `extraction_state`; the reviewer's transcript read is the only place alliance is assessed. Do not prescribe rebuilding a parallel monitor; re-entry is consumer-first, and the preferred path is detecting the signal inside the main Jove call. Supersedes both the 2026-06-03 "alliance model made executable" note and the 2026-06-09 version of this note. Confirmed 2026-07-09.
