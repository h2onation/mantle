...
Purpose: remove merged worktrees and their branches without killing any worktree that currently has an active Codex session running against it.

## Active-session detection

Every Codex session writes a `.jsonl` transcript to `~/.Codex/projects/<slug>/` where `<slug>` is the worktree's absolute path with `/` and `.` both replaced by `-`.

Examples:
- `/Users/jeffwaters/mywalnut` → `-Users-jeffwaters-mywalnut`
- `/Users/jeffwaters/mywalnut/.Codex/worktrees/brave-banach` → `-Users-jeffwaters-mywalnut--Codex-worktrees-brave-banach`
...
1. `cd /Users/jeffwaters/mywalnut` (main repo root — never run cleanup from inside a worktree).
...
4. For each worktree other than the main repo root:
   ...
   b. Check `~/.Codex/projects/<slug>/` for any `.jsonl` file modified in the last 2 hours:
      ```bash
      find ~/.Codex/projects/<slug> -name "*.jsonl" -mmin -120 2>/dev/null
      ```
   ...
   e. `cd /Users/jeffwaters/mywalnut` to re-anchor (git worktree remove can reset cwd).
...
- **Never delete the main repo worktree** (`/Users/jeffwaters/mywalnut`). It's never in the deletion list.
- **Never touch an active worktree's files, branch, or remote branch.** Active = `.jsonl` modified in the last 2 hours.
- **Never delete session directories under `~/.Codex/projects/`.** Those belong to Codex, not git. They can be left alone even when the worktree they correspond to has been removed.
