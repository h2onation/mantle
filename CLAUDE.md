...
1. **The voice** — ONE voice for every module (ADR-054 removed ADR-053's per-module prompt fork): the admin `conductor_prompt` override ("Tuning" page) → `CONDUCTOR_PROMPT`. Nothing module-side can replace it — modules carry a **brief** instead (`modules.brief`, edited at `/admin/modules`): founder-written steering appended in `call-persona.ts` as a labeled `## This conversation's focus` section that COMPOSES with the voice, chat path only (the compose/checkpoint call never sees it), added AFTER the conductor SHA stamp so the Tuning score trend stays per-conductor. Both voice rungs are save-guarded by the protected lines (crisis 988/741741, the `---reflection-ready---` marker, the reflection-writing section) — `CONDUCTOR_REQUIRED_FRAGMENTS` / `validateConductorPromptEdit` in conductor-prompt.ts are the one source of truth for enforcement and admin display; the brief needs no fragment guard (the voice always arrives intact) and only rejects standalone `---marker---` lines (`validateModuleBrief` in modules.ts).
