...
- **Tier 2 — Voice and behavior.** Base + persona-delta architecture. Base in `src/lib/persona/voice-scaffold.ts` (VOICE_INTRO_PARAGRAPHS_BASE, VOICE_RULES_BASE (21 rules — takes positions on truth not should with a safety carve-out for crisis, refuses the phantom baseline, sometimes names strength in the same mechanism, variance from responsiveness; R-17 split into restraint + understanding-not-prelude, R-18 split into refuse-phantom + sometimes-name-strength to prevent the model auto-attaching strength to every refusal), EXAMPLE_REGISTER_BASE, LANDING_EXAMPLES_BASE, WEAK_STRONG_EXAMPLES_BASE, BANNED_PHRASES, BANNED_PATTERNS, plus scaffolded sections: DASH_TO_PERIOD_RULE, LANDING_INTRO, DEEPENING_INTRO/OUTRO, PACING_RULE, WHEN_JOVE_IS_WRONG, WHEN_USER_ASKS_WHAT_SHOULD_I_DO (never-prescribe with one safety exception — Jove DOES prescribe contact with 988 + Crisis Text Line when the user produces crisis signals)). Per-persona trait deltas in `voice-{autistic,adhd,dyslexic,general}.ts` — each adds only the persona signature on top of base. Phantom-baseline persona forms live in deltas (autism = social baseline; ADHD = care-as-execution; dyslexic = medium/format mismatch, shipping as hypothesis). `composeTier2` in system-prompt.ts emits base + each active persona's delta. Edit voice-scaffold.ts for cross-persona changes; edit a specific persona module only for that persona's signature.
