...
- **Tier 2 — Voice and behavior.** Base + persona-delta architecture. Base in `src/lib/persona/voice-scaffold.ts` (VOICE_INTRO_PARAGRAPHS_BASE, VOICE_RULES_BASE (16 rules — sparring partner with forensic backing), EXAMPLE_REGISTER_BASE, LANDING_EXAMPLES_BASE, WEAK_STRONG_EXAMPLES_BASE, BANNED_PHRASES, BANNED_PATTERNS, plus scaffolded sections: DASH_TO_PERIOD_RULE, LANDING_INTRO, DEEPENING_INTRO/OUTRO, PACING_RULE, WHEN_JOVE_IS_WRONG, WHEN_USER_ASKS_WHAT_SHOULD_I_DO). Per-persona trait deltas in `voice-{autistic,audhd,dyslexic,general}.ts` — each adds only the persona signature on top of base. `composeTier2` in system-prompt.ts emits base + each active persona's delta. Edit voice-scaffold.ts for cross-persona changes; edit a specific persona module only for that persona's signature.
