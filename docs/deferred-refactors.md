...
Across two simplification passes plus three focused follow-up sessions:
...
- **Spawned session — `requireUser()` helper**: replaced 18 inline auth checks
  in API routes with a shared helper mirroring `requireAdmin()`.
- **Spawned session — SSE parser dedup**: new `parseAnthropicStream` helper
...
- **Spawned session — `BuildPromptOptions` disjoint union**: refactored
  `buildSystemPrompt` to take a discriminated union (`group` vs `oneOnOne`)
  with type-narrowed branches.
- **Spawned session — `buildTier3` refactor**: 350-line if/else ladder
  replaced with a `Tier3Block[]` data structure; each block is independently
...
- **Status**: Not a simplification — it's an additive feature. Excluded from
  the cleanup passes by scope.
