...
1. **`tier1` — the voice.** One document per conversation, resolved by the module voice ladder (`resolveModulePrompt`): the conversation's module `custom_prompt` when set → the admin `conductor_prompt` override → the `CONDUCTOR_PROMPT` code constant. A module with no custom prompt runs the shared conductor (the default remains ONE voice — ADR-053 amends ADR-052); every custom prompt passes the same protected-fragment guard at save. Stable across turns → sits at the front of the cached prefix.
