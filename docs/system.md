...
There is one entry shape. Sections are the founder-authored **modules** (ADR-053) and can hold many entries — no cap, no type discriminator. An entry's `section` is the module slug of the conversation it came from, **code-assigned at compose time** (the model no longer picks); composition writes the draft; the user edits and confirms. `tags` is a closed `{strength}` set; `layer` is a frozen nullable legacy column. Nothing orphans: a disabled module keeps its Manual section visible while entries remain (`buildModuleGroups` is the one grouping function — Manual page, Home counts, PDF, admin view). See rules.md "Checkpoint and Manual Entry Voice" for composition quality rules and word count range (80–300).
...
1. **`tier1` — the voice.** ONE voice for every module (ADR-054): the admin `conductor_prompt` override → the `CONDUCTOR_PROMPT` code constant. Nothing module-side replaces it. A module's optional **brief** (`modules.brief`) is appended to this block's text in `call-persona.ts` as a labeled `## This conversation's focus` section — it COMPOSES with the voice, chat path only (the compose/checkpoint call never sees it), and is added AFTER the conductor SHA stamp so score-trend grouping stays per-conductor. Stable across turns → the whole block sits at the front of the cached prefix.
...
- **`feature_gates`** — global on/off switches. ONE live key since the modules cutover: `extraction_brief` (OFF skips the extraction call entirely — debug only). The per-mode door gates were deleted 2026-07-15 (ADR-053); a module's own `enabled` flag (the `modules` table, `/admin/modules`) is the only door switch. TEMPORARY scaffolding with a documented deletion condition.
