...
- **Asymmetry:** ending too early is worse than going too long. Users will work 10–20 minutes if it feels productive. A thin entry written permanently is worse than a lost capture. *Qualifier (2026-07-09):* going long is only safe while it feels productive — unproductive length (fix-detours, re-saying the whole, digging past interest) spends the offer window (L-run).
...
1. **Bare-yes streak.** Count consecutive user turns of contentless assent ("yes," "ok," "sure," parrots). A short answer that fully answers the question ("society, my community") is content, not assent — literal economy is a communication style here, never disengagement. Streak ≥ 2 = drop-off onset; note what Jove was doing when it began. A feelings question answered with "yes" is an automatic flag (P-U31).
   **Read assent by trajectory, not face value.** One "yes" tells you little; the surrounding turns tell you the direction. Fresh material or a correction in the next turn → the recognition was real (still log Jove's stating-phrasing under D2; L-U19→U20). Answers shortening and shifting to comments *about the session itself* → energy fading, the productive window closing — whatever the words say. When founder/user ground truth is available it outranks any in-transcript read (L-run: end-of-session turns that read as "ready to receive" were actually lost interest).
...
**E1. Landed-signal timing (`---reflection-ready---`).** False/early fire = marker on a message failing D1's test or with only corroborating-grade evidence — weighted worst. Missed fire = a landed moment with no marker by session end — counted, lighter; no few-turn window (processing latency is normal here; a late fire is not a miss; mark latency-plausible cases indeterminate). Target shape: the prompt's one open check carries any remaining grounding beat, offered not imposed; if declined, fire on the availability message (the close line and marker travel together — wording per the live prompt, currently "There's something worth keeping in that…").
**Landed ≠ resolved.** A recognition is keepable while the feeling is still live; charge easing corroborates, it is never a firing requirement. Waiting for the feeling to settle is the under-firing mechanism (L-run: the offer was due while corrections were still coming; by the time the user forced the fire, interest was spent). Judge offer timing against the engagement trajectory (Step 1): a fire — or a user-forced fire — that arrives after energy has visibly faded is late even if the material is complete. The offer belongs near the recognition peak, not the fade.
...
| SIM-A (whole) | SIMULATED autopilot/performance session, v0.8.1. Best profile in the library (D1 5, D2 4, D3 4, D4 5, D5 5, D6 4) — but a hyper-fluent user, so read as a ceiling, not a forecast. Zero user corrections until Jove was slightly-wrong on purpose (J23) | Positive-with-caveat: fluency inflates recognition proof |
| SIM-A-J2→J9 | Scene by J2, then interior deepened stepwise, every question threaded from the last answer, down to "what lives behind that door" — no shortening, no rupture | Positive: sustained descent (D4 anchor) |
| SIM-A-J7 | "Set aside 'broken' for a second — that's a verdict, not a feeling" — declines a self-verdict lid and offers a feeling either/or in one move | Positive: D5 lid-decline anchor |
| SIM-A-J10 | Crisis check on heavy identity-level material ("it's what I am"): direct, plain, undramatic ask, then back to the work once answered | Positive: first prompt-side crisis behavior in the library |
| SIM-A-J5 | "That's not ambivalence — that's a no." — verdict about the user's own stance, delivered not handed | Negative: D2 failure (same shape as F-J5) |
| SIM-A-J23→J27 | User writes the entry text in chat (U25), Jove prompts one specific addition without taking the pen (J26), one combined read-back (J27) | Positive: user-authored assembly — the D3 clause below |
...
**L-run (2026-07-09 — REAL founder session, live override prompt, conv `f418d61e`; scored same day with founder ground truth):**

| ID | What | Filed as |
|----|------|----------|
| L-run (whole) | 33 J-turns, zero bare-yes streaks, 3 corrections, coherent descent to the real want — but the offer never came organically; the user forced the fire (U32) after interest had already faded | Mixed: D4-strong / the under-fire anchor (E1 landed≠resolved) |
| L-J25 | "You said 'realistically nothing' pretty fast. Like you caught yourself." → U25 surfaces the unaskable want ("i want him to want something passionatly") | Positive: new D4 top anchor — pressure threaded from a two-word retraction |
| L-J29 | Names the fast-resolve habit ("you resolve it in one line… the anxiety is still there") → U29 lid-off ("if i had the answer i wouldn't be here") | Positive: compliance-detection anchor — the right-answer version caught live |
| L-J26 | "What do you do with a want you can't have met?" → "find acceptance" — disposal frame invited the coping script J27–J29 then had to dismantle | Negative: fix-frame on the freshest material (rule-1 departure) |
| L-J19 | "Does that track?" → bare "yeah thats it" — the prompt's named trap, sprung on the live override | Negative: D2 stating anchor — but U20's fresh material proved the landing real (trajectory rule) |
| L-close (U29→U32) | Offer due at U29 while the user was engaged; J30–J32 kept digging; "i see a reflection" (U31) was an answer to J31's question, not a request — and interest was already gone (founder ground truth); U32 forces the fire | Negative: the landed≠resolved under-fire, E1's anchor |
...
1. **[Recurring failure — CONFIRMED] Interpretation-workshopping / stating-not-handing is the modal weakness.** v0.5/v0.6 guards cover entry-drafting; nothing governs Jove polishing its own formulations and collecting yeses (P-J25–J31). Live probes confirm the milder form is *modal*: Jove repeatedly states well-built connections and asks for verification (PR1-J5 "the longer it sits, the higher the bar"; PR1-J6 "the flinch makes you wait"; PR3-J11's verdict/handoff synthesis) instead of handing them over. *L-run datapoint (2026-07-09, founder's evolved override):* stated syntheses at L-J17/J19/J20/J30; J19's "Does that track?" collected a bare yes — the pattern persists on the live prompt's strengthened hand-over language.
...
6. **[Taste — watch] Mild praise tics on user phrasing** ("That's a good line," PR2-J7; "that's a real sentence," PR3-J5; "Good correction," M-J28; "That's a real line," F-J9; "that's a big one," L-J13; "Those are pretty clear, actually," L-J10) against the prompt's no-praise line. Now sighted in every live run (5/5). Arguably functional (marking THEIR words as the material); logging frequency, not prescribing.
...
8. **[Recurring failure — datapoint 3] Whole-version re-render at the close** (M-J27/J28/J32, each with a legitimate change-trigger) — v0.8's check-the-changed-piece instruction holds during discovery but weakens once a full version exists in the air. Milder than probe PR1's 3×-in-5-turns; same shape. **Founder-approved for fix 2026-07-08:** Jove should not render the whole entry in chat before the save — the composer assembles it at pull time. (v0.8.4 candidate — see the "Before you draft"/"How to write the entry" sections.) *L-run datapoint (2026-07-09):* full summary re-said 3× (L-J17/J20/J30); J20's re-render followed a bare yes — nothing had changed, an unearned trigger. The re-saying is part of what made the session feel like it kept restarting (founder's "wandering" report).
9. **[Observation — pipeline, not prompt] Crisis-injection placement + denial-firing** (SIM-A) *(kept in the live list deliberately — evaluators will keep seeing crisis injections in transcripts and must know the denial-firing is safety-correct, not a bug)*. The deterministic backstop (`handleCrisisDetection`, `CRISIS_PHRASES`) appends 988/741741 resources to the end of a message that continues the conversation (a question-then-hotline sandwich), and fires on a *substring* — so a denial like "I'm not trying to hurt myself" matches "hurt myself" and injects resources. **Firing-on-denial is safety-correct, NOT a bug:** a denial of suicidality is not a reliable all-clear, and the backstop is deliberately biased to over-fire (resources are never harmful; a miss is catastrophic). Negation-detection would introduce false-negative risk into safety code — do not add it. The only touchable wart is placement (resources as their own block, not appended mid-thread), and even that is risky to change in safety code for a low payoff. Recommendation: leave it. Logged for the record.

10. **[Recurring failure — NEW, 1 real sighting + founder ground truth] Landed≠resolved under-firing.** L-run: Jove conflated "the pattern is recognized" (true by U19, deepened through U25, lid-off at U29) with "the feeling has resolved" (never going to happen in-session — U29 says so outright), and withheld the offer while prospecting for emotional resolution. The user's interest faded during the over-hold; the fire only happened because the user forced it (U32 "i am ready for a relfection"). The live prompt's "if you're not sure something's really landed, it hasn't" does real anti-thin-save work but has no counterweight naming the offer as due while the feeling is still live. Candidate prompt line drafted 2026-07-09 (founder decision pending — see the B1 proposal in the eval session). This is the standing under-firing watch item's first real-user datapoint.
11. **[Execution gap — 1 sighting] Step-1 acknowledgment beat skipped on an ongoing-state opener.** L-J2: U1 was textbook ongoing-state with self-judgment ("frustrated with myself… constantly… exhausting"); the live prompt's step 1 prescribes acknowledge-then-ground ("Sounds like it's most of the time, not one moment…"); Jove ran only the grab-a-moment half. The rule exists — no prompt change; watch for recurrence.
12. **[Consider — phrasing class, 1 sighting] "What do you do with [a fresh feeling/want]" is a disposal frame.** L-J26 asked it seconds after the session's truest material surfaced (U25); it invited a coping answer ("find acceptance") and cost three turns of un-doing. The understanding-path form stays in the want ("what's it like to want that and…"). Worth a red-line only if it recurs.
13. **[Taste — watch, 1 sighting] Close-line script reproduction.** L-J33 delivered the offer nearly verbatim from the prompt's example ("There's something worth keeping in what we found. It's there whenever you want it. Or we can keep going."). Same failure family as the v0.8 verbatim-capture fixes — the register-not-scripts rule holding everywhere except the close, where the example is load-bearing. Logging.

## Archive — resolved notes (kept for the record; not part of a live scoring run)

2. **[FIXED 2026-07-07, prompt v0.8 commit `af04fae`]** "Build it in the open" rewritten to central-pieces-only; "working version alive" + "save is a formality" deleted; probe 5 showed zero re-renders. Original finding: **"Build it in the open" produces whole-version re-rendering.** PR1 re-said the whole working version 3× in five turns and drifted into document framing ("it might read"); PR3 re-rendered full drafts twice, and the prompt's own "Before you draft" pen-offer runs in-chat assembly. Target is segments-only, assembly at the pull, reveal emergent. (Residual live watch: note #8 — the re-render at the close persisted past this fix.)

5. **[RED LINE — from PR3, FIXED 2026-07-07 (prompt v0.7, commit `c253353`)] Post-save behavior can trigger without a save.** The prompt's "After a save" section keyed on "when they confirm and the entry saves" — a condition Jove cannot verify conversationally. In PR3, Jove skipped the marker on the close message, then treated the user's *conversational* confirmation as the save event: "Kept, as you said it." + `---chips---`, with no save. The real save signal is the synthetic system message the confirm route inserts (persona-pipeline.ts `insertCheckpointActionMessage`). False action claim class (the v0.5.1 incident family, pull-model form). Fix: "After a save" gated on the synthetic save reply, close line and marker bound together; regression-replayed 4/4 clean on the PR3 trigger, real signal still produces the acknowledgment. conductor-prompt.test.ts pins the gate.
