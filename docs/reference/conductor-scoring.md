...
8. **[Recurring failure — datapoint 2] Whole-version re-render at the close** (M-J27/J28/J32, each with a legitimate change-trigger) — v0.8's check-the-changed-piece instruction holds during discovery but weakens once a full version exists in the air. Milder than probe PR1's 3×-in-5-turns; same shape. **Founder-approved for fix 2026-07-08:** Jove should not render the whole entry in chat before the save — the composer assembles it at pull time. (v0.8.4 candidate — see the "Before you draft"/"How to write the entry" sections.)
