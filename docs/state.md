...
- Group chat (Parts 6a-6e shipped 2026-04-01): Complete. Detection, intro, facilitation, message gate, participant removal, edge case hardening all deployed. Webhook subscription includes participant.added/removed and chat.created events. Two migrations applied: linq_group_chats table + conversations.linq_group_chat_id column + intro_sent_at column (unused but harmless). Gate thresholds (GATE_MIN_MESSAGES=3, GATE_NUDGE_MESSAGES=6) may need tuning after watching real conversations — check GROUP_SAGE_CALL logs. UX polish (2026-04-01): sender labeling (Mantle user shows profile name, others show phone), prompt identity section for natural name learning, gate simplified (removed first-after-intro special case), re-detection for inactive groups when "sage" mentioned.
