...
## ADR-054: One Voice, Module Briefs — the Per-Module Prompt Fork Is Removed

**Date**: 2026-07-21
**Status**: Accepted

**Context**: ADR-053 gave each module an optional full custom Jove prompt — the top rung of the voice ladder, a deliberate per-module fork save-guarded by `CONDUCTOR_REQUIRED_FRAGMENTS`. Planning the first real module set for the ND launch, the founder called the opposite need: the voice must be CONSTANT across every module (so Tuning edits propagate everywhere), with module specifics authored separately in admin. A senior-engineer review of the fork found it was also **half-wired**: the chat path read `custom_prompt` (`call-persona.ts`) but the entry-compose path never did (`buildPromptOptionsFromContext` never set `modulePrompt`), so a forked module would talk in one voice and write its Manual entries in another. No prod row ever carried a custom prompt (the module set was still empty).

**Decision**:

1. **One voice.** The ladder is again conductor-only (ADR-052's original shape): admin `conductor_prompt` override → code `CONDUCTOR_PROMPT`. Nothing module-side can replace it.
2. **Module brief.** `modules.brief` (nullable text, capped at the shared 4000-char prose cap) is founder-written steering — what the module is about, what to listen for, how to open — appended to the system prompt as a labeled section (`## This conversation's focus`) that COMPOSES with the voice. Injected in `call-persona.ts` AFTER the conductor SHA stamp (the Tuning score trend stays grouped by conductor version, not fragmented by module) and inside the cached block-1 prefix (no per-turn cost). **Chat path only** — the compose/checkpoint call never sees the brief: the entry is written from the transcript, which already embodies wherever the brief steered the conversation, and a write-time brief would bias entries toward the door's agenda instead of what actually surfaced.
3. **Guard simplification.** The brief needs no required-fragment guard — the voice (crisis lines, markers, entry-writing spec) always arrives with the conductor and cannot be edited away per module. Validation is a length cap plus rejection of standalone `---marker---` lines (`validateModuleBrief`), so a brief can never carry marker machinery.
4. **Deleted**: `modules.custom_prompt` (column, migration `20260721120000`), `resolveModulePrompt`, `validateModulePrompt`, the admin full-prompt editor with its "start from the current conductor" button and fork-drift warning, and the `conductorText` plumbing in the admin GET whose only consumer was that button.

**Consequences**: Amends ADR-053 point 2; restores ADR-052's "sole voice" in full. Tuning edits reach every module instantly; the fork-drift risk ADR-053 accepted is structurally gone, as is the talk-in-one-voice/write-in-another inconsistency. Module authoring gets lighter (a paragraph, not a prompt). If a module ever genuinely needs a different VOICE or a different entry-writing form, that is a new founder decision — design it then; nothing is pre-built for it. Field lesson recorded: any per-module prompt injection must be wired in BOTH the chat path and the compose path, or deliberately scoped to one — the fork's half-wiring went unnoticed for six days because the table was empty.
