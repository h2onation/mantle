...
export async function generateSessionSummary(
  ...
  try {
    const response = await anthropicFetch({
      ...
      max_tokens: 512,
      system:
        `Summarize this conversation between a user and ${PERSONA_NAME} (an AI building behavioral models). Your response MUST begin with a short title on the first line in this exact format:\nTITLE: [3-8 word descriptive title]\n\nThen a blank line, then the summary. The title names what the USER brought and explored — their topic, in words close to their own (e.g. "TITLE: Conflict avoidance at work" or "TITLE: Wanting a dog"). Never title it after ${PERSONA_NAME}'s opening question or framing: if ${PERSONA_NAME} opened with one topic and the user steered to another, the title follows the user. No quotes or ending punctuation in the title.\n\nFor the summary: focus on topics explored, what the user revealed, checkpoints confirmed, what was left unresolved. Refer to the user as "they" — never assume gender. Keep under 300 words. This summary will be injected into ${PERSONA_NAME}'s context next session.`,
