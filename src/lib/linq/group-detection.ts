...
const INTRO_NO_ACCOUNTS =
  `This is ${PERSONA_NAME_FORMAL} by ${BRAND.name}. I don't have any connected accounts in this group. ` +
  `If you have a ${BRAND.name} account, connect your number at ${BRAND.domain}`;

const INTRO_MULTI_USER =
  `Hey, I'm ${PERSONA_NAME_FORMAL}. I know some of you from ${BRAND.name}, but in a group I keep ` +
...
async function sendIntroduction(
  ...
  const introText = firstName
    ? `Hey, I'm ${PERSONA_NAME_FORMAL} by ${BRAND.name}. I'll use what I know about ${firstName}'s patterns to ask better questions, but I won't share details from private conversations.`
    : `Hey, I'm ${PERSONA_NAME_FORMAL} by ${BRAND.name}. I'll use what I know to ask better questions, but I won't share details from private conversations.`;
